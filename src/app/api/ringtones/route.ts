...
export async function GET() {
    ...
    try {
        ...
        const ringtones = products.data.map((product) => {
            ...
            // Force 2026/Valentine/New Year tracks to be NEW and Future-dated to ensure top sorting
            if (title.toLowerCase().includes('valentine') || title.toLowerCase().includes('new year') || title.includes('2026') || realReleaseDate > Date.now()) {
