...
export default function RadioLivePage() {
    ...
    // NUCLEAR FLUSH VERSION: V12 - Zero Flip Signal Lock
    const STATION_VERSION = "RADIO_V12_SIGNAL_LOCK";
    ...
    useEffect(() => {
        const fetchStationData = async () => {
            try {
                ...
                // 2. SIGNAL LOCK V12: Pre-Flight Check
                // Filter out any track that failed signing on the server
                const pool = rawPool.filter(t => t.audioUrl && t.audioUrl.length > 20);

                // 3. Fisher-Yates Shuffle (Uniform Randomness)
                ...
                // 4. Persistence / Cache Flush Check
                ...
                if (savedFingerprint === fingerprint && savedPlaylistStr) {
                    console.log("[Nashville-UI] Signal Lock active. Resuming sequence.");
                    ...
                    console.log(`[Nashville-UI] V12 Reset: Calibrating ${pool.length} tracks.`);
                    finalPlaylist = shuffle([...pool]); 
